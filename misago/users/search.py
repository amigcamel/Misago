...
class SearchUsers(SearchProvider):
    ...
    def search(self, query, page=1):
        ...
        return {'results': UserSerializer(results, many=True).data, 'count': len(results)}


def search_users(**filters):
    User = get_user_model()
    queryset = User.objects.order_by('slug').select_related('rank', 'ban_cache', 'online_tracker')
