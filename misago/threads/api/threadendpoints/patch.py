...
from ...permissions import (
    allow_add_participants, allow_add_participant, allow_change_owner, allow_edit_thread,
...
def patch_top_category(request, thread, value):
    ...
    categories = list(
        Category.objects.all_categories().filter(id__in=request.user.acl['visible_categories'])
    )
